...
class report_manager {
  ...
  public function extended_ordersHTML($rs)
  {
    ...
    natsort($uf_list);
    $strHTML = "<table border='1'><thead><tr><th><p class='textAlignLeft'>{$Text['product_name']}</p></th>";
  ...
  public function write_summarized_orders_html($id, $the_date)
  {
      ...
      while ($row = $rs->fetch_assoc()) {
          $html .=
              ...
              . ($row['custom_product_ref'] ? 
                 '<p class="custom_product_ref">' . $row['custom_product_ref'] . '</p>'
                 : '&nbsp;')
              . "</td>\n"
